...
-(void)btnClick:(UIButton *)button{
    if (button.tag == 98) {
        ...
        if (self.sortType == buddleSort) {
            [[JFSortingHelper sharedInstance]buddleSort:self.data];
            [self showSortedArray:[JFSortingHelper sharedInstance].finalArray];
            
            
        }else if (self.sortType == quickSort){
            [[JFSortingHelper sharedInstance]quickSortWithArray:self.data];
            [self showSortedArray:[JFSortingHelper sharedInstance].finalArray];
        
        }else if (self.sortType == selectSort){
            [[JFSortingHelper sharedInstance]selectSort:self.data];
            [self showSortedArray:[JFSortingHelper sharedInstance].finalArray];
            
        }else if (self.sortType == inserSort){
            [[JFSortingHelper sharedInstance]inserSort:self.data];
            [self showSortedArray:[JFSortingHelper sharedInstance].finalArray];
            
        }else if (self.sortType == shellSort){
            [[JFSortingHelper sharedInstance]ShellSort:self.data dk:(int)self.data.count];
            [self showSortedArray:[JFSortingHelper sharedInstance].finalArray];
...
-(void)showSortedArray:(NSMutableArray *)array{
